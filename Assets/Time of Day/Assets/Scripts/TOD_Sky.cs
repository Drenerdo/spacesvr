...
public partial class TOD_Sky : MonoBehaviour
{
	...
	private void UpdateCelestials()
	{
		...
		// Transform updates
		{
			...
			float sunSize = 2.0f * Mathf.Tan(4.0f * 0.5f * Mathf.Deg2Rad * Sun.MeshSize);
		...
		// Color calculations
		{
			...
			// Ground color
			float dayGroundColorMult = Day.ColorMultiplier;
			Color dayGroundColor = TOD_Util.MulRGB(Day.AmbientColor.Evaluate(time), dayGroundColorMult);

			float nightGroundColorMult = 0.25f * Night.ColorMultiplier;
			Color nightGroundColor = TOD_Util.MulRGB(Night.AmbientColor.Evaluate(time), nightGroundColorMult);

			GroundColor = Color.Lerp(nightGroundColor, dayGroundColor, LerpValue);

			// Ambient color
			float dayAmbientColorMult = Day.ColorMultiplier;
			Color dayAmbientColor = TOD_Util.MulRGB(Day.AmbientColor.Evaluate(time), dayAmbientColorMult);

			float nightAmbientColorMult = 0.50f * Night.ColorMultiplier;
			...
			// Moon halo color
			float moonHaloMult = 0.25f * Night.ColorMultiplier * moonAltitude;
